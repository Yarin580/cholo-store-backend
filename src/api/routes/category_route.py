...
async def get_categories(category_id: int = None, category_name: str = None):
    return CategoryService().get_categories(category_id=category_id, category_name=category_name)

@category_route.get("/{category_id}", response_model=CategoryResponseDto)
async def get_category(category_id: int):
    return CategoryService().get_category_by_id(category_id=category_id)
...
async def create_category(category_to_create: CategoryCreateDto,
    ...
    return CategoryService().create_category(category_dto=category_to_create)
